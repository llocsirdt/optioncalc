...
function v7Signal(A, prior, ctx = {}) {
  ...
  // (0b) BE WRONG (v7) — open the OPPOSITE side EARLY (before the full structural flip) when a confirmed
  //      opposite reversal prints against the held book: a reversal candle AND breaking the prior
  //      candle's high/low (the user's "bearish engulfing / breaking lows / crossed the centerline"
  //      reads). We do NOT force-cover the held (underwater) side — the opposite is a HEDGE that also
  //      profits if the move continues; the held side keeps its own cover chances, and the engine's
  //      risk cap bounds total exposure ("be patient"). Needs the bidirectional engine.
  if (cfg.beWrong && held !== 'none') {
    if (held === 'bull' && lowerLow && pat.bearishReversalCandle(c15, prior15)) return { openSide: 'bear', cover: false, reason: 'be-wrong→bear (breaking lows + bearish candle)' };
    if (held === 'bear' && higherHigh && pat.bullishReversalCandle(c15, prior15)) return { openSide: 'bull', cover: false, reason: 'be-wrong→bull (breaking highs + bullish candle)' };
  }
