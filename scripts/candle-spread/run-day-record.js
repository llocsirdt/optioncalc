...
function daysIn(dir) {
  if (!_daysCache.has(dir)) _daysCache.set(dir, load5mDays(dir));
...
function runDayRecord({ date, variant, symbol }) {
  if (!/^\d{4}-\d{2}-\d{2}$/.test(String(date || ''))) { const e = new Error('date=YYYY-MM-DD required'); e.code = 'BAD_DATE'; throw e; }
  const hit = findDay(date);
  if (!hit) {
    const e = new Error(`No 5m dataset covers ${date}. Checked: ` + DIRS.map((d) => path.basename(d.dir)).join(', '));
    e.code = 'NO_DATASET'; e.checked = DIRS.map((d) => path.basename(d.dir)); throw e;
  }
