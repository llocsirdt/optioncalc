...
(async () => {
  ...
  for (const r of list) {
    ...
    try {
      ...
      fs.writeFileSync(path.join(ARCHIVE, `${r.runId}.json`), JSON.stringify(j, null, 2));
      saved++;
      console.log(`  saved ${r.runId}  (events ${(j.events || []).length}, realized $${j.state?.realizedPnl})`);
    } catch (e) { failed++; console.log('  ERROR', r.runId, e.message); }
  }
  console.log(`\nsynced ${saved} run(s) to ${ARCHIVE}${failed ? ` (${failed} failed)` : ''}`);
})();
