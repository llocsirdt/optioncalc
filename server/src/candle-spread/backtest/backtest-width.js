...
// LAZY. This used to load the whole dataset at MODULE LOAD, which is fine for a CLI but wrong for a
// library: importing makeGeo/makeAdaptiveGeo should not touch the disk, and on the deployed server the
// dataset does not exist at all (only server/ ships), so an eager read crashed the require. Only the CLI
// paths below actually need the days.
let _days = null;
const days_ = () => (_days || (_days = load5mDays(di >= 0 ? process.argv[di + 1] : eng.DATA_DIR)));
...
function avgDebit(geo) {   // sample the ATM open debit across days (first bar of each) → % of width
  let s = 0, n = 0;
  for (const d of days_()) { const b = d.bars[0], A = b.analysis; const iv = eng.bs.ivFromRelBandWidth((A['15m'].bbupper - A['15m'].bblower) / A['15m'].close); s += geo.buildOpen('bull', A['5m'].close, eng.bs.tauFromTime(b.dt), iv).limit; n++; }
  return s / n;
}
...
if (require.main === module) {
  ...
  console.log(`SPREAD-WIDTH EXPERIMENT — ${days_().length} days, 5m engine, QTY=1\n`);
  for (const sig of [['v6', v6], ['be-wrong', bw]]) {
    ...
    for (const [label, geo] of geos) {
      ...
      const arr = days_().map(d => runDay5m(d.bars, sig[1], opts).terminal), s = stat(arr);
