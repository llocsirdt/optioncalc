...
(function(global) {
  ...
  // rankCandidateScore/gapRealizationFactor/findShortStrike live in
  // shared/portfolio-risk.js so findPortfolioHedgeCandidates (the aggregate
  // N-leg search) scores and sorts candidates identically to this curated
  // single-spread search — a candidate surfaced in both panels should rank
  // (and look) the same way in both.
  function evaluateCandidate(label, family, positionLegs, candidateLegs, positionWidth, candidateWidth, meta) {
    ...
    const strikeGapWidth = Math.abs(PortfolioRisk.findShortStrike(positionLegs) - PortfolioRisk.findShortStrike(candidateLegs));
